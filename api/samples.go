...
// @produce     json
// @success     200 {array} database.Sample "All samples which respect the given conditions"
...
func SamplesGet(ctx *fiber.Ctx) error {
	...
	builder := database.Instance().Select("*").From("samples")
	if len(sensorId) > 0 {
	...
	samples, err := database.Multiple[models.Sample](
		context.Background(),
		sql,
		args...,
	)
